...
contract Staking is Ownable, TokenPrice {
    ...
    function getTokenPrice(address tokenAddress, uint128 amount) public view returns (uint256) {
        return Oracle.estimateAmountOut(tokenAddress, amount, 10);
    }
    ...
    function getTotalStakedTokens() internal view returns(uint256) {
        (int256 usdtPrice, int256 usdcPrice, int256 ethPrice, int256 maticPrice) = getLatestPrice();
    ...
    function getRewardsPerMinute(address _token) public view returns(uint256) {
        uint256 apy = getApr(_token);
        (int256 usdtPrice, int256 usdcPrice, int256 ethPrice, int256 maticPrice) = getLatestPrice();
        uint256 userStake = stakes[msg.sender][_token].amount;
        uint256 userStakeInUSD = 0;
        for(uint i = 0; i < allowedTokens.length; i++) {
